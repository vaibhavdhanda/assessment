...
public class TestEngine {
    private static java.util.logging.Logger LOGGER = Logger.getLogger(TestEngine.class.getName());
    static {
    ...
    public void validateAskPriceIsGreaterThanBidPrice(){
        ...
        while(iter.hasNext()){
            ...
            Assert.assertTrue(getDoubleFromString(bestAskPrice) > getDoubleFromString(bestBidPrice), "BestAskPrice is not greater than BestBidPrice in message: " + message);
        }
    }
    ...
    public void validateEndTimeIsAfterBeginTime() {
        ...
        while(iter.hasNext()){
            ...
            Assert.assertTrue(getDoubleFromString(endTime) > getDoubleFromString(beginTime), "OHLC: End Time is not greater than Begin Time in message: " + message);
        }
    }
    ...
    public void validateOhlcOpenAndClosePricesAreWithInDailyHighAndLow() {
        ...
        while(iter.hasNext()){
            ...
            Assert.assertTrue(closePrice <= highPrice && closePrice >= lowPrice, "OHLC: Close Price is not within daily high and low prices message: " + message);
        }
    }
    ...
    public void validateTriggeringOrderSideIsCorrect() {
        ...
        while(iter.hasNext()){
            ...
            for (int i=0; i < tradeCount ; i++){
                ...
                Assert.assertTrue(tradeSide != null && ("s".equals(tradeSide) || "b".equals(tradeSide)), "Trade: Side does not match 'b' or 's'. Message = " + message);
            }
        }
    ...
    public void validateTriggeringOrderTypeIsCorrect() {
        ...
        while(iter.hasNext()){
            ...
            for (int i=0; i < tradeCount ; i++){
                ...
                Assert.assertTrue(orderType != null && ("m".equals(orderType) || "l".equals(orderType)), "Trade: Order Type does not match 'm' or 'l'. Message = " + message);
            }
        }
    ...
    private void assertEquals(String fieldName, String actualValue, String expectedValue){
        ...
        Assert.assertEquals(actualValue, expectedValue, String.format("Actual %s doesn't match expected ticker. Actual=%s and Expected=%s", fieldName, actualValue, expectedValue));
